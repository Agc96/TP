...
void leerRangoFechas(int &fechaIni, int &fechaFin){
    ...
    while(true){
        cout<<"Ingrese la fecha inicio del rango para considerar en el reporte de ventas: (dd/mm/aaaa) ";
        cin>>dd>>c>>mm>>c>>aaaa;
        fechaIni=aaaa*10000+mm*100+dd;
        cout<<"Ingrese la fecha fin del rango para considerar en el reporte de ventas: (dd/mm/aaaa) ";
        cin>>dd>>c>>mm>>c>>aaaa;
        fechaFin=aaaa*10000+mm*100+dd;
...
void leerVentas(int* arrCodLibro, double* arrPrecioLibro, int *arrCantidadLibro, double *arrRecaudadoLibro,
        int cantLibros, int fechaIni, int fechaFin){
    int dd, mm, aaaa, pos;
    char c;
    ...
    while(true){
        ...
        archVentas>>c>>mm>>c>>aaaa>>codLibro>>cantVenta;
    ...
    for(int i=0; i<cantLibros; i++){
        ...
        for (int j=0; j<=cantRegAutores; j++){
            if (arrAutorCodLibro[j]==arrCodLibro[i]){
                //Mostrar autor
                archReporte<<setw(4)<<" "<<arrCodAutor[j]<<setw(17)<<arrRegaliaAutorLibro[j]<<"%"<<setw(22)<<arrRegaliaAutorLibro[j]*arrRecaudadoLibro[i]/100<<endl;
            }
        }
    }
